...
public final class Banner {
    ...
    public static void display(PrintWriter out) {
        try {
            File jarvizDir = new File(System.getProperty("user.home"));
            String envJarvizDir = System.getenv("JARVIZ_DIR");
            if (envJarvizDir != null && !envJarvizDir.isEmpty()) {
                File dir = new File(envJarvizDir);
                if (dir.exists()) {
                    jarvizDir = dir;
                }
            }

            File parent = new File(jarvizDir, "/.jarviz/caches");
            File markerFile = getMarkerFile(parent, BANNER);
            if (!markerFile.exists()) {
                if (!JarvizOutput.isQuiet()) out.println(BANNER.banner);
