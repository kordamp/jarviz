...
import org.kordamp.jarviz.commands.EntriesExtractCommand;
...
public class EntriesExtract extends AbstractJarvizSubcommand<Entries> {
    @CommandLine.ArgGroup
    public Exclusive exclusive;

    public static class Exclusive {
        @CommandLine.Option(names = {"--entry-name"}, required = true, paramLabel = "<name>")
        public String entryName;

        @CommandLine.Option(names = {"--entry-pattern"}, required = true, paramLabel = "<pattern>")
    ...
    @CommandLine.Option(names = {"--target-directory"}, paramLabel = "<directory>")
    public Path targetDirectory;

    @Override
    protected int execute() {
        return new EntriesExtractCommand().execute(EntriesExtractCommand.config()
