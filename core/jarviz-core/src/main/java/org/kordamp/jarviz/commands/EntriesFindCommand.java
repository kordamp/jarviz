    ...
    private void output(Configuration configuration, JarProcessor.JarFileResult<Set<String>> result) {
        if (!result.getResult().isEmpty()) {
            ...
            if (isBlank(configuration.getEntryPattern())) {
                configuration.getOut().println($$("entries.find.entry.name", configuration.getEntryName()));
            } else {
                configuration.getOut().println($$("entries.find.entry.pattern", configuration.getEntryPattern()));
            }
